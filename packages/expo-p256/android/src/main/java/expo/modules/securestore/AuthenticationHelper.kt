...
class AuthenticationHelper(
  ...
  suspend fun authenticateCipher(cipher: Cipher, requiresAuthentication: Boolean, title: String): Cipher {
    if (requiresAuthentication) {
      return openAuthenticationPromptForCipher(cipher, title).cryptoObject?.cipher
        ?: throw AuthenticationException("Couldn't get cipher from authentication result")
    }
    return cipher
  }

  suspend fun authenticateSignature(signature: Signature, requiresAuthentication: Boolean, title: String): Signature {
    if (requiresAuthentication) {
      return openAuthenticationPromptForSignature(signature, title).cryptoObject?.signature
        ?: throw AuthenticationException("Couldn't get signature from authentication result")
    }
    return signature
  }

  private suspend fun openAuthenticationPromptForCipher(
    cipher: Cipher,
    title: String
    ...
    return withContext(Dispatchers.Main.immediate) {
      try {
        return@withContext authenticationPrompt.authenticateCipher(cipher)
  ...
  private suspend fun openAuthenticationPromptForSignature(
    signature: Signature,
    title: String
  ): BiometricPrompt.AuthenticationResult {
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) {
    ...
    return withContext(Dispatchers.Main.immediate) {
      try {
        return@withContext authenticationPrompt.authenticateSignature(signature)
          ?: throw AuthenticationException("Couldn't get the authentication result")
      } finally {
        isAuthenticating = false
      }
