...
import expo.modules.p256.EncryptException
import expo.modules.p256.KeyStoreException
...
class HybridAESEncryptor(private var mContext: Context, private val mAESEncryptor: AESEncryptor) : KeyBasedEncryptor<KeyStore.PrivateKeyEntry> {
  ...
  override fun getExtendedKeyStoreAlias(options: P256Options, requireAuthentication: Boolean): String {
    val suffix = if (requireAuthentication) {
      P256Module.AUTHENTICATED_KEYSTORE_SUFFIX
    } else {
      P256Module.UNAUTHENTICATED_KEYSTORE_SUFFIX
    }
    return "${getKeyStoreAlias(options)}:$suffix"
  }

  override fun getKeyStoreAlias(options: P256Options): String {
    val baseAlias = options.keychainService
    return "$RSA_CIPHER:$baseAlias"
  }

  @Throws(GeneralSecurityException::class)
  override fun initializeKeyStoreEntry(keyStore: KeyStore, options: P256Options): KeyStore.PrivateKeyEntry {
