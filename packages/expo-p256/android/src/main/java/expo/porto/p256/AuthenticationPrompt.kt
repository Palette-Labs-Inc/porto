...
class AuthenticationPrompt(private val currentActivity: FragmentActivity, context: Context, title: String) {
  ...
  suspend fun authenticateCipher(cipher: Cipher): BiometricPrompt.AuthenticationResult? =
  ...
  suspend fun authenticateSignature(signature: Signature): BiometricPrompt.AuthenticationResult? =
    suspendCoroutine { continuation ->
      BiometricPrompt(
        currentActivity,
        executor,
