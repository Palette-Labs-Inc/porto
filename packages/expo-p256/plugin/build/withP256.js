...
const pkg = require('@porto/expo-p256/package.json');
...
const withP256 = (config, { faceIDPermission, configureAndroidBackup = true } = {}) => {
    ...
    (0, config_plugins_1.withAndroidManifest)(config, (config) => {
        ...
        if (!configureAndroidBackup) {
            backupConfig === BACKUP_RULES_PATH &&
        ...
        console.warn('Expo-p256 tried to apply Android Auto Backup rules, but other backup rules are already present. ' +
            'Refer to the Expo-secure-store docs (https://docs.expo.dev/versions/latest/sdk/securestore/) to configure your backup rules.');
        return config;
    });
    return config;
};
exports.default = (0, config_plugins_1.createRunOncePlugin)(withP256, pkg.name, pkg.version);
