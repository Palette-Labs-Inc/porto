...
> = (config, { faceIDPermission, configureAndroidBackup = true } = {}) => {
  ...
  withAndroidManifest(config, (config) => {
    ...
    const dataExtractionRules =
      mainApplication.$["android:dataExtractionRules"]; // SDK >= 31
    ...
    const canApplyBackupConfig =
      !backupConfig || backupConfig === BACKUP_RULES_PATH;
...
export default createRunOncePlugin(withP256, pkg.name, pkg.version);
