...
export function getExports({ onEntry, } = {}) {
    const dist = {};
    const src = {};
    const entries = fs
    ...
    for (const parentEntry of entries) {
        if (!parentEntry.isDirectory()) {
            if (parentEntry.name.endsWith('test.ts'))
                continue;
        ...
        const entries = fs.readdirSync(resolve(parentEntry.parentPath, parentEntry.name), {
            withFileTypes: true,
        });
