 ...
 */
export function from(implementation) {
    return {
        ...implementation,
 ...
 * @returns Implementation.
 */
export function local(parameters = {}) {
    ...
    return from({
        actions: {
            ...
            async createAccount(parameters) {
                ...
                const { account, context, signatures } = await (async () => {
                    ...
                    if (parameters.context && parameters.signatures)
                        return {
                            account: parameters.context.account,
            ...
            async loadAccounts(parameters) {
                ...
                const keys = await Promise.all(Array.from({ length: Number(keyCount) }, (_, index) => Delegation.keyAt(client, { account: address, index })));
            ...
            async revokeKey(parameters) {
                ...
                if (key.role === 'admin' &&
                    ...
                    throw new Error('cannot revoke key. account must have at least one admin key.');
                await Delegation.execute(client, {
                    account,
            ...
            async signPersonalMessage(parameters) {
                ...
                const key = account.keys?.find((key) => key.role === 'admin' && key.canSign);
                if (!key)
...
export function dialog(parameters = {}) {
    const { host = 'https://exp.porto.sh/dialog', renderer = Dialog.iframe() } = parameters;
        ...
        actions: {
            ...
            async prepareCreateAccount(parameters) {
                ...
                if (request.method !== 'experimental_prepareCreateAccount')
                    throw new Error('Cannot prepare create account for method: ' + request.method);
                const provider = getProvider(store);
                return await provider.request(request);
            },
            async revokeKey(parameters) {
...
async function prepareCreateAccount(parameters) {
    const { address, authorizeKeys, client, defaultExpiry, keystoreHost } = parameters;
    const label = parameters.label ?? `${address.slice(0, 8)}\u2026${address.slice(-6)}`;
...
function getAuthorizeCalls(keys) {
    return keys.flatMap((key) => {
        ...
        if (key.role === 'session' &&
            ...
            !permissions?.spend)
            throw new Error('session key must have at least one permission (`permissions`).');
...
async function getAuthorizedExecuteKey(parameters) {
    ...
    if (key) {
        const key = account.keys?.find((key) => key.publicKey === parameters.key?.publicKey && key.canSign);
        if (!key)
            throw new Error(`key (publicKey: ${parameters.key?.publicKey}) does not exist or is not a provider-managed key.`);
        return key;
    }
    ...
    const sessionKey = account.keys?.find((key) => {
        ...
        if (key.expiry < BigInt(Math.floor(Date.now() / 1000)))
            return false;
        const hasValidScope = key.permissions?.calls?.some((scope) => calls.some((call) => {
    ...
    // Fall back to an admin key.
    const adminKey = account.keys?.find((key) => key.role === 'admin' && key.canSign);
...
async function getKeysToAuthorize(parameters) {
    ...
    return await Promise.all(authorizeKeys
        .map(async (k) => {
