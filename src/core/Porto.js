...
export function create(parameters = {}) {
    const { announceProvider = defaultConfig.announceProvider, chains = defaultConfig.chains, implementation = defaultConfig.implementation, storage = defaultConfig.storage, transports = defaultConfig.transports, } = parameters;
    const store = createStore(subscribeWithSelector(persist((_) => ({
        accounts: [],
        chain: chains[0],
        requestQueue: [],
    }), {
            ...
            return {
                accounts: state.accounts.map((account) => ({
                    ...
                    keys: account.keys?.map((key) => ({
                        ...key,
                        privateKey: typeof key.privateKey === 'function'
                            ? undefined
                            : key.privateKey,
                    })),
                })),
