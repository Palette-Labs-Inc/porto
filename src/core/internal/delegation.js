...
import { getEip712Domain as getEip712Domain_viem, readContract, } from 'viem/actions';
...
export async function execute(client, parameters) {
    ...
    const { request, signatures } = await (async () => {
        ...
        const { request, signPayloads: payloads } = await prepareExecute(client, parameters);
        return {
            request,
    ...
    // signature.
    const opData = AbiParameters.encodePacked(['uint256', 'bytes'], [nonce, executeSignature]);
    try {
...
export async function prepareExecute(client, parameters) {
    const { account, delegation, executor, nonce = Hex.toBigInt(Hex.random(32)), ...rest } = parameters;
    const calls = parameters.calls.map((call) => ({
        data: call.data ?? '0x',
