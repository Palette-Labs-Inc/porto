...
export function from(parameters) {
        ...
        async request(request_) {
            ...
            switch (request.method) {
                ...
                case 'eth_sendTransaction': {
                    if (state.accounts.length === 0)
                        throw new ox_Provider.DisconnectedError();
                    const [{ chainId, data = '0x', from, to, value = '0x0' }] = request.params;
                    ...
                    const account = state.accounts.find((account) => Address.isEqual(account.address, from));
                    if (!account)
                ...
                case 'experimental_prepareCreateAccount': {
                    ...
                    const client = getClient();
                    const { context, signPayloads } = await implementation.actions.prepareCreateAccount({
                        address,
                        authorizeKeys,
                        label,
                        internal: {
                ...
                case 'experimental_revokeKey': {
                    ...
                    const keys = account.keys?.filter((key) => key.publicKey !== publicKey);
                    store.setState((x) => ({
                        ...x,
                    ...
                    const { accounts } = await (async () => {
                        ...
                        try {
                            ...
                            if (address && credentialId)
                                return await implementation.actions.loadAccounts(loadAccountsParams);
                            throw error;
                        }
                    })();
                    store.setState((x) => ({ ...x, accounts }));
                ...
                case 'wallet_getCallsStatus': {
                    ...
                    if (!receipt)
                        return { receipts: [], status: 'PENDING' };
                    return {
                        receipts: [receipt],
                        status: 'CONFIRMED',
                    };
    ...
    function setup() {
        const unsubscribe_accounts = store.subscribe((state) => state.accounts, (accounts) => {
            emitter.emit('accountsChanged', accounts.map((account) => account.address));
        });
        const unsubscribe_chain = store.subscribe((state) => state.chain, (chain) => {
